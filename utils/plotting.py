...
def plot_multiple_algorithms(env, rew, n_bins, y_label = "", title = ""):
    assert len(env) == len(rew)
    ...
    for k in range(len(env)):
        ...
        # standard error
        std = std / np.sqrt(len(xss))

        plt.figure(figsize=(10, 6))

        colours = ['red', 'green', 'blue', 'purple', 'brown', 'pink', 'gray', 'cyan', 'magenta']

        plt.plot(bins_x, avg, label='Average', color=colours[k])
        plt.fill_between(bins_x, avg - std, avg + std, color=colours[k], alpha=0.2, label='Standard Error')

    plt.xlabel('Environment steps')
    plt.ylabel(y_label)
    plt.title(title)
    # plt.legend(("IDS"), 'upper left')
    plt.grid(True)
    plt.show()
...
def plot_binned_line_with_std(xss, yss, n_bins, y_label = "", title = "", plot_individuals = False):
    ...
    if plot_individuals:
        ...
        for a in avgs:
            ...
            plt.plot(bins_x, a, linestyle=':', linewidth=1, alpha=0.7, color=colours[c % 10])
            c += 1
