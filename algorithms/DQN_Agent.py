...
class ReplayMemory:
    ...
    def sample(self, batch_size):
        ...
        actions = torch.as_tensor([self.actions[i] for i in indices], dtype=torch.long, device=device).to(device)
        next_states = torch.stack([torch.as_tensor(self.next_states[i], dtype=torch.float32, device=device) for i in indices]).to(device)
        rewards = torch.as_tensor([self.rewards[i] for i in indices], dtype=torch.float32, device=device).to(device)
        dones = torch.as_tensor([self.dones[i] for i in indices], dtype=torch.bool, device=device).to(device)
...
class DQN_Agent:
    ...
    def select_action(self, state, training=True):
        ...
        if (np.random.random() < self.epsilon_max) and training:
